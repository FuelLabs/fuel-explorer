...
### A6. Popularity-weighted caching (Task 35)

A `hot_keys(kind, key, hits, last_seen)` table in the same sqlite file counts requests per account, transaction id and block height. Hits decay by half once a day. The ranking feeds two places: disk-cache eviction skips blocks referenced by the top 50 accounts and top 200 transactions, and `transactionsByOwner` pages served from fuel-core are cached per `(account, cursor)` with a TTL of 30 s times the account's hits, capped at 1 h. The 48-hour index is unaffected.
