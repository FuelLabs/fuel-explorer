...
type ContractLinksProps = {
  project: Project | null;
  links: MetadataLink[] | undefined;
};

export function ContractLinks({ project, links = [] }: ContractLinksProps) {
  const hasLinks = links.length > 0;
  const hasAnySocial = project?.twitter || project?.github || project?.discord;

  if (!project || (!hasLinks && !hasAnySocial)) {
    return null;
  }
