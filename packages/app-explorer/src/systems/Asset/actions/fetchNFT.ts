...
export const fetchNFT = act(schema, async (input) => {
  const provider = new Provider(FUEL_CHAIN.providerUrl);

  const { assetId, contractId } = input;
  ...
  const [name = '', symbol = '', supply, decimals] = result.value;
