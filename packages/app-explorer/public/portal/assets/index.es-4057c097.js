  ...
  class l extends n.IHeartBeat {
    constructor(v) {
      super(v),
        ...
        (this.interval = v?.interval || a.HEARTBEAT_INTERVAL);
    }
    static init(v) {
...
function I1() {
  ...
  function i(v) {
    return Object.assign(Object.assign({}, v), {
      level: v?.level || c.PINO_LOGGER_DEFAULTS.level,
    });
  }
...
function Wh(c = Tl.randomBytes(B1)) {
...
let Z1 = class {
  ...
  emitError(i) {
    const r = this.parseError(
      new Error(
        i?.message || `WebSocket connection failed for host: ${Jh(this.url)}`
      )
    );
...
(function (c, i) {
  ...
  (_e[et] =
    ...
    (_e[v] =
      ...
      _e[te] =
        !1);
  var It = typeof ta == 'object' && ta && ta.Object === Object && ta,
...
class ob {
  ...
  async setKeyChain(i) {
    await this.core.storage.setItem(this.storageKey, Ll(i));
  }
  async getKeyChain() {
    const i = await this.core.storage.getItem(this.storageKey);
    return typeof i < 'u' ? ql(i) : void 0;
  }
  async persist() {
  ...
  constructor(i, r, n) {
    (this.core = i),
      ...
      (this.encode = async (a, l, d) => {
        ...
        return tm({ type: E, symKey: y, message: w, senderPublicKey: C });
      }),
      ...
      (this.getPayloadSenderPublicKey = (a) => {
        const l = Oh(a);
        return l.senderPublicKey ? ga(l.senderPublicKey, nm) : void 0;
      }),
      (this.core = i),
      (this.logger = Se.generateChildLogger(r, this.name)),
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              (d = l?.internal) != null && d.throwOnFailedPublish)
            )
...
class Ob extends T1 {
  ...
  formatTarget(i) {
    if (typeof i == 'string') return lm(i);
    if (typeof i == 'number') return fm(i);
...
class Ab extends N1 {
  constructor(i, r) {
    super(i, r),
      ...
      (this.init = async (n) => {
        if (this.verifyDisabled || pm() || !Dl()) return;
        const a = this.getVerifyUrl(n?.verifyUrl);
  ...
  constructor(i) {
    ...
    const r =
      typeof i?.logger < 'u' && typeof i?.logger != 'string'
        ? i.logger
        : Se.pino(
            Se.getDefaultLoggerOptions({ level: i?.logger || T_.logger })
          );
    (this.logger = Se.generateChildLogger(r, this.name)),
      (this.heartbeat = new ds.HeartBeat()),
      (this.crypto = new cb(this, this.logger, i?.keychain)),
      (this.history = new Cb(this, this.logger)),
      (this.expirer = new Ob(this, this.logger)),
      (this.storage =
        i != null && i.storage
          ? i.storage
          : new g1(gl(gl({}, N_), i?.storageOptions))),
      (this.relayer = new Eb({
        core: this,
        logger: this.logger,
        relayUrl: this.relayUrl,
...
class Bb extends D1 {
  constructor(i) {
    super(i),
      ...
      (this.request = async (r) => {
        ...
        return (
          ...
          await Promise.all([
            ...
            new Promise(async (C) => {
              const A = await ym(this.client.core.storage, vl);
              mm({ id: v, topic: l, wcDeepLink: A }), C();
            }),
            w(),
          ]).then((C) => C[2])
        );
      ...
      (this.find = (r) => (
        this.isInitialized(),
        this.client.session.getAll().filter((n) => wm(n, r))
      )),
      ...
      (this.isValidConnect = async (r) => {
        ...
        if ((_i(n) || (await this.isValidPairingTopic(n)), !_m(v, !0))) {
          const { message: w } = Y(
            'MISSING_OR_INVALID',
            `connect() relays: ${v}`
          );
      ...
      (this.isValidReject = async (r) => {
        ...
        const { id: n, reason: a } = r;
        if ((await this.isValidProposalId(n), !Em(a))) {
          const { message: l } = Y(
            'MISSING_OR_INVALID',
      ...
      (this.isValidSessionSettleRequest = (r) => {
        ...
        const v = xm(a, 'onSessionSettleRequest()');
      ...
      (this.isValidRespond = async (r) => {
        ...
        try {
          ...
          throw (
            ((n = r?.response) != null && n.id && this.cleanupAfterResponse(r),
            d)
          );
        }
        if (!Om(l)) {
  ...
  registerExpirerEvents() {
    this.client.core.expirer.on(Sr.expired, async (i) => {
      const { topic: r, id: n } = Fl(i.target);
...
let Jb = class pf extends F1 {
  constructor(i) {
    super(i),
      ...
      (this.name = i?.name || rc.name),
      (this.metadata = i?.metadata || gm());
    const r =
      typeof i?.logger < 'u' && typeof i?.logger != 'string'
        ? i.logger
        : Se.pino(
            Se.getDefaultLoggerOptions({ level: i?.logger || rc.logger })
          );
    (this.core = i?.core || new Fb(i)),
      (this.logger = Se.generateChildLogger(r, this.name)),
...
function uE(c) {
  ...
  for (const [r, n] of Object.entries(c)) {
    const a = n.methods || [],
      l = n.events || [],
      d = n.accounts || [],
      v = Ml(r) ? [r] : n.chains ? n.chains : df(n.accounts);
  ...
  constructor(i) {
    (this.events = new mc()),
      ...
      (this.logger =
        typeof i?.logger < 'u' && typeof i?.logger != 'string'
          ? i.logger
          : Se.pino(Se.getDefaultLoggerOptions({ level: i?.logger || wl }))),
...
function DE(c) {
  ...
  const w = {
    ...
    y = l?.filter((j) => !vc.includes(j)),
    E = n?.filter((j) => !gc.includes(j));
  if (!r && !d && !a && !(y != null && y.length) && !(E != null && E.length))
    return { required: i.length ? w : void 0 };
  const C = (y?.length && E?.length) || !r,
    A = {
...
class Cc {
  ...
  async connect(i) {
    ...
    try {
      ...
      if (!a) return;
      const l = $m(a.namespaces, [this.namespace]);
  ...
  getRpcConfig(i) {
    var r, n;
    const a = (r = i?.chains) != null ? r : [],
      l = (n = i?.optionalChains) != null ? n : [],
      d = a.concat(l);
    if (!d.length)
      throw new Error(
        'No chains specified in either `chains` or `optionalChains`'
      );
    const v = a.length ? i?.methods || gc : [],
      w = a.length ? i?.events || vc : [],
      y = i?.optionalMethods || [],
      E = i?.optionalEvents || [],
      C = i?.rpcMap || this.buildRpcMap(d, i.projectId),
      A = i?.qrModalOptions || void 0;
    return {
      chains: a?.map((j) => this.formatChainId(j)),
  ...
  async initialize(i) {
    if (
      ...
      let r;
      try {
        const { WalletConnectModal: n } = await Fm(
          () => import('./index-bc266d71.js').then((a) => a.i),
          ['assets/index-bc266d71.js', 'assets/index-4d435dca.js']
        );
        r = n;
      } catch {
        throw new Error(
  ...
  loadConnectOpts(i) {
    ...
    r &&
      ...
      n &&
        ...
        (this.rpc.optionalChains = n?.map((l) => this.formatChainId(l))),
        n.forEach((l) => {
          this.rpc.rpcMap[l] = a?.[l] || this.getRpcUrl(l);
        }));
  }
  ...
  async loadPersistedSession() {
    ...
    this.setChainIds(i ? [this.formatChainId(i)] : r?.accounts),
