...
export class BlockStore {
  ...
  // a restart). Only `<digits>.json` files count; `.tmp` files from an
  ...
  private scanDiskBytesSync(): void {
    ...
    const entries: { height: number; size: number; mtime: number }[] = [];
  ...
  async evictDisk(): Promise<number> {
    ...
    entries.sort((a, b) => (a?.mtime ?? 0) - (b?.mtime ?? 0));
  ...
  private async evictOverflow(): Promise<void> {
    while (
      ...
      try {
        await fs.unlink(this.diskPath(height));
      } catch {
        /* already gone */
      }
    }
  ...
  private async loadFromSource(height: number): Promise<GQLBlock | null> {
    ...
    return this.opts.decode?.(bytes);
  }

  private async readDisk(height: number): Promise<GQLBlock | null> {
    try {
      return JSON.parse(
        await fs.readFile(this.diskPath(height), 'utf8'),
      ) as GQLBlock;
    } catch {
      return null;
    }
