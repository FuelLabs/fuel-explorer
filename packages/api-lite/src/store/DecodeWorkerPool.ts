...
// Under tsx and jest the worker is still .ts and needs tsx's require hook.
...
// A Buffer under 4 KiB shares a slab; transferring that slab would detach
// every other Buffer on it, so a partial view is copied first.
...
// One block per worker at a time, so a crash maps to exactly one load.
