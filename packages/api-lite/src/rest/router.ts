...
// Same reasoning as ChartsRouteDeps: GET /dashboard needs no L1 ingestion.
...
export async function handleRestRequest(
  ...
  if (path === '/dashboard') {
    try {
      ...
      // 5s matches nginx's `proxy_cache_valid 200 5s` on `/api/dashboard`
      // and the resolver's DataCache TTL, so all three expire together.
