...
// Shared by the `getBlocksDashboard` GraphQL resolver and the `GET
// /dashboard` REST route, so both surfaces compute (and DataCache-cache,
// under the same 'getBlocksDashboard' key the GraphQL resolver always used)
// an identical payload.
export async function buildBlocksDashboard(
  ctx: AppContext,
...
export const analyticsResolvers = {
  Query: {
    async getBlocksDashboard(_: unknown, __: unknown, ctx: AppContext) {
      return buildBlocksDashboard(ctx);
    },
