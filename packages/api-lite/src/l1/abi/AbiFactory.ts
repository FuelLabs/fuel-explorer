// Copied from packages/graphql/src/application/uc/IndexL1/abi/AbiFactory.ts.
// Production types this as ethers' InterfaceAbi; api-lite uses viem only, so
// this returns the raw ABI array (cast to viem's Abi at the decodeEventLog
// call site, since the literals below aren't declared `as const`).
...
export default class AbiFactory {
  static create(network: string, contract: string) {
    const ABI: { [network: string]: { [contract: string]: unknown } } = {
      mainnet: {
